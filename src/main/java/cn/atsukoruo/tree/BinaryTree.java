...
public class BinaryTree<T extends Comparable<T>> {
    ...
    public void print() {
        ...
        while (true) {
            while (node != null) {
                System.out.printf("%s: {left: %5s, right: %5s, color: %s}%n", node.data,
                        node.leftChild == null ? null : node.leftChild.data,
                        node.rightChild == null ? null : node.rightChild.data,
                        node.color);
